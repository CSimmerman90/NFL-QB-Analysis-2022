...
data = pd.read_csv('afc_s_qb_total_2021.csv')
...
# Get QB data
jax_data = get_qb_data(data_radar, 'JAX')
hou_data = get_qb_data(data_radar, 'HOU')
ind_data = get_qb_data(data_radar, 'IND')
ten_data = get_qb_data(data_radar, 'TEN')
# Plot QB data
ax1 = create_radar_chart(ax1, angles, jax_data, team_colors['JAX'])
ax2 = create_radar_chart(ax2, angles, hou_data, team_colors['HOU'])
ax3 = create_radar_chart(ax3, angles, ind_data, team_colors['IND'])
ax4 = create_radar_chart(ax4, angles, ten_data, team_colors['TEN'])
