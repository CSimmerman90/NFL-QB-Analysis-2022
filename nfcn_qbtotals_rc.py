...
data = pd.read_csv('nfc_n_qb_total_2021.csv')
...
# Get QB data
min_data = get_qb_data(data_radar, 'MIN')
chi_data = get_qb_data(data_radar, 'CHI')
det_data = get_qb_data(data_radar, 'DET')
gnb_data = get_qb_data(data_radar, 'GNB')
# Plot QB data
ax1 = create_radar_chart(ax1, angles, min_data, team_colors['MIN'])
ax2 = create_radar_chart(ax2, angles, chi_data, team_colors['CHI'])
