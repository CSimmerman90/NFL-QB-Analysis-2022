...
data = pd.read_csv('top_qb_totals.csv')

# Select stat categories
categories = ['Cmp', 'Att', 'Cmp%', 'Yds', 'TD', 'Int']
...
def create_radar_chart(ax, angles, player_data, color='blue'):
    ...
    # Set axis limits
    ax.set(xlim=(0, 2*np.pi), ylim=(0, 1)) #THIS IS THE CODE I NEED TO CHANGE
...
# Get QB data
nwe_data = get_qb_data(data_radar, 'NWE')
mia_data = get_qb_data(data_radar, 'MIA')
sfo_data = get_qb_data(data_radar, 'SFO')
...
ax3 = create_radar_chart(ax3, angles, sfo_data, team_colors['SFO'])
