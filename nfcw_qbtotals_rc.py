...
data = pd.read_csv('nfc_w_qb_total_2021.csv')
...
ari_data = get_qb_data(data_radar, 'ARI')
sea_data = get_qb_data(data_radar, 'SEA')
lar_data = get_qb_data(data_radar, 'LAR')
# Plot QB data
ax1 = create_radar_chart(ax1, angles, sfo_data, team_colors['SFO'])
ax2 = create_radar_chart(ax2, angles, ari_data, team_colors['ARI'])
ax3 = create_radar_chart(ax3, angles, sea_data, team_colors['SEA'])
ax4 = create_radar_chart(ax4, angles, lar_data, team_colors['LAR'])
