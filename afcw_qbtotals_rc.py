...
data = pd.read_csv('afc_w_qb_total_2021.csv')
...
# Get QB data
oak_data = get_qb_data(data_radar, 'OAK')
lac_data = get_qb_data(data_radar, 'LAC')
kan_data = get_qb_data(data_radar, 'KAN')
den_data = get_qb_data(data_radar, 'DEN')
# Plot QB data
ax1 = create_radar_chart(ax1, angles, oak_data, team_colors['OAK'])
ax2 = create_radar_chart(ax2, angles, lac_data, team_colors['LAC'])
ax3 = create_radar_chart(ax3, angles, kan_data, team_colors['KAN'])
