...
data = pd.read_csv('afc_e_qb_total_2021.csv')
...
# Get QB data
buf_data = get_qb_data(data_radar, 'BUF')
nwe_data = get_qb_data(data_radar, 'NWE')
mia_data = get_qb_data(data_radar, 'MIA')
nyj_data = get_qb_data(data_radar, 'NYJ')
# Plot QB data
ax1 = create_radar_chart(ax1, angles, buf_data, team_colors['BUF'])
ax2 = create_radar_chart(ax2, angles, nwe_data, team_colors['NWE'])
